# Create a shader graph and use it with a material | Shader Graph | 17.6.0

Source: [Original](https://docs.unity3d.com/Packages/com.unity.shadergraph@17.6/manual/First-Shader-Graph.html)
...
* [Add and connect nodes in a shader graph](Create-Node-Menu.html)
...
2. Name the created shader graph asset and press Enter.

The [Shader Graph window](Shader-Graph-Window.html) opens, which allows you to edit the shader graph in the created asset. If the window doesn't open, double-click on the created asset.
...
2. In the **Create Node** menu's search bar, type `color`.
3. In the **Input** > **Basic** category, double-click on **Color**.
...
2. Drag it to the **Base Color** block port of the **Fragment** section of the [master stack](Master-Stack.html).
...
2. Use the color picker to change the color.
...
1. Open the shader graph you created earlier in the [Shader Graph window](Shader-Graph-Window.html).
2. In the [Blackboard](Blackboard.html), select **Add (+)**, and then select **Color**.

   The Blackboard now displays a [property of Color type](Property-Types.html#color).
3. Select the property.
4. In the [Graph Inspector](Internal-Inspector.html), in the **Node Settings** tab:
...
5. Drag the property from the Blackboard onto the Shader Graph window's workspace.
6. Connect the property's node to the **Base Color** block port of the **Fragment** section of the [master stack](Master-Stack.html), instead of the Color node you were using previously.
...
7. Save your graph, and return to the material's Inspector.
...
Older tutorials use a former version of Shader Graph with master nodes. To know the differences between the former master node and the [Master Stack](Master-Stack.html), refer to the [Upgrade Guide](Upgrade-Guide-10-0-x.html).