# Upgrade to version 10.0.x of Shader Graph | Shader Graph | 17.6.0

Source: [Original](https://docs.unity3d.com/Packages/com.unity.shadergraph@17.6/manual/Upgrade-Guide-10-0-x.html)
...
Shader Graph has renamed the previous Sample Cubemap Node to [Sample Reflected Cubemap Node](Sample-Reflected-Cubemap-Node.html), and has added a new [Sample Cubemap Node](Sample-Cubemap-Node.html), which uses world space direction.

## Master Stack graph output

Shader Graph has removed the Master Nodes and introduced a more flexible [Master Stack](Master-Stack.html) solution for defining graph output in 10.0. You can still open all graphs created in previous versions, because Shader Graph automatically upgrades them. This page describes the expected behavior and explains when you might need to perform manual upgrade steps.
...
Shader Graph introduced an internal [Graph Inspector](Internal-Inspector.html) in version 10.0. The Graph Inspector is a floating window that displays settings related to objects you select in the graph.
...
Previously, some nodes and properties such as the [Color Node](Color-Node.html) didn't behave as intended, but they now work correctly in Shader Graph version 10.0. Older graphs that rely on the incorrect behavior still function the same as before, and you can choose to individually upgrade any deprecated nodes and properties. If you don't enable **Allow Deprecated Behaviors** in [Shader Graph Preferences](Shader-Graph-Preferences.html), newly-created nodes and properties use the latest version node and property behaviors.
...
For deprecated properties, **(Deprecated)** appears after the property name in the [Blackboard](Blackboard.html).

![The deprecated Color property in the blackboard, as an example.](images/DeprecatedColorProperty.png)

When you select a deprecated node or property, a warning appears in the [Graph Inspector](Internal-Inspector.html) along with an **Update** button that allows you to upgrade the selection. You can use undo/redo to reverse this upgrade process.

![The deprecation message as it appears in the Graph Inspector.](images/DeprecatedWarning.png)

If you enable **Allow Deprecated Behaviors** in [Shader Graph Preferences](Shader-Graph-Preferences.html), Shader Graph displays the version of the deprecated node or property, and doesn't display any warnings even though the **Update** button appears. You can also use the Blackboard or Searcher to create deprecated nodes and properties.