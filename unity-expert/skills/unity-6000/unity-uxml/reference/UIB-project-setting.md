...
To ensure that newly created documents are, by default, compatible with Editor UI and that the UI Builder library displays the additional controls for Editor UI, select **Enable Editor Extension Authoring by Default**. For more information, refer to [Enable Editor Extension Authoring for UI Documents (UXML)](UIB-interface-overview.md#enable-editor-extension-authoring-for-ui-documents-uxml).
