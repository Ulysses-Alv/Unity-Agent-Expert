# Gather Texture 2D node | Shader Graph | 17.6.0

Source: [Original](https://docs.unity3d.com/Packages/com.unity.shadergraph@17.6/manual/Gather-Texture-2D-Node.html)
...
The Gather Texture 2D node can only be connected to a Block node in the **Fragment** Context. For more information on Block nodes and Contexts, refer to [Master Stack](Master-Stack.html).

## Inputs

The Gather Texture 2D node has the following input ports:
...
The Gather Texture 2D node has the following output ports:
...
The following nodes are related or similar to the Gather Texture 2D node:
