# Precision Modes | Shader Graph | 17.6.0

Source: [Original](https://docs.unity3d.com/Packages/com.unity.shadergraph@17.6/manual/Precision-Modes.html)
...
To set the precision of an entire graph, select the [**Graph Settings**](Graph-Settings-Tab.html) tab in the [Graph Inspector](Internal-Inspector.html) and adjust the **Precision** control. Select a node in your graph and select the **Node Settings** tab in the Graph Inspector to adjust the precision of individual nodes.
...
To visualize data precision in a graph, set the [**Color Mode**](Color-Modes.html) control to **Precision**. This applies color coding to your nodes:
...
All nodes use the **Inherit** precision mode by default. In this mode, a node that has one or more edge connections takes on the precision mode of an incoming [edge](Edge.html). Nodes that do not have any edge connections take on **Graph Precision**. If you change the **Graph Precision** mode, the precision of those nodes also changes.
...
Nodes with no input ports, such as [Input nodes](Input-Nodes.html), inherit the **Graph Precision**. However, complex inheritance rules still affect other nodes in the same group, as illustrated in the figure below.
...
Precision behavior and user interface elements for [Sub Graphs](Sub-graph.html) and their nodes do not differ from other graphs and nodes. Sub Graphs represent a function, and you can affect that function's inputs, outputs, and operators by modifying the relevant set of precision settings.
...
To manually determine the precision of **Sub Graph Inputs**, open the [Graph Inspector](Internal-Inspector.html) and set precision modes for each individual [Property](Property-Types.html). Properties that use the Inherit option take on the **Graph Precision** you set for the Sub Graph.

#### Sub Graph Precision within other graphs

By default, a Sub Graph has a Precision Mode of `Switchable`. You can modify Precision Mode of any [Sub Graph node](Sub-graph-Node.html) for that Sub Graph, as long as you set the Precision Mode on the Sub Graph as `Switchable`.
