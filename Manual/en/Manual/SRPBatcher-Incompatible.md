...
In some rare cases, you might want to intentionally make particular **GameObjects** incompatible with the SRP Batcher. For example, if you want to use [GPU instancing](GPUInstancing.md), which isn’t compatible with the SRP Batcher. If you want to render many identical meshes with the exact same material, GPU instancing can be more efficient than the SRP Batcher. To use GPU instancing, you must either:

* Use [Graphics.RenderMeshInstanced](../ScriptReference/Graphics.RenderMeshInstanced.md).
...
**Tip**: If you use GPU instancing instead of the SRP Batcher, use the [Profiler](Profiler.md) to make sure that GPU instancing is more efficient for your application than the SRP Batcher.
...
2. Add a new [material property](SL-Properties.md) declaration into the shader’s `Properties` block. Don’t declare the new material property in the `UnityPerMaterial` constant buffer.
