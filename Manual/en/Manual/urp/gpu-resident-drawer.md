...
* The [Forward+](rendering/forward-rendering-paths.md) **rendering path**.
...
* Build times are longer because Unity compiles all the `BatchRendererGroup` shader variants into your build.
  The **Probe Atlas Blending** is used by default when both the [Forward+](rendering/forward-rendering-paths.md) rendering path and the GPU Resident Drawer are used.
...
* [Rendering Debugger](features/rendering-debugger-reference.md).
...
**Note:** The GPU Resident Drawer uses its own GPU **Occlusion culling** system, but supports [Dynamic Occlusion](../occlusion-culling-dynamic-gameobjects.md) as well.
