...
A camera stack consists of a [Base Camera](../camera-types-and-render-type-introduction.md#base-camera) and one or more [Overlay Cameras](../camera-types-and-render-type-introduction.md#overlay-camera). A camera stack overrides the output of the Base Camera with the combined output of all the cameras in the camera stack. As a result, anything you can do with the output of a Base Camera, you can do with the output of a camera stack. For example, you can render a camera stack to a render target, or apply **post-processing** effects.
