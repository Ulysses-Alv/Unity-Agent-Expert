...
At the start of its render loop, an [Overlay Camera](camera-types-and-render-type-introduction.md#overlay-camera) receives a color buffer containing color data from the previous Cameras in the Camera stack. It does not clear the contents of the color buffer.
...
1. Unity gets the list of all active [Base Cameras](camera-types-and-render-type-introduction.md#base-camera) in the scene.
...
4. For each Base Camera that renders to a Render Texture, Unity performs the following steps:
   ...
   3. For each [Overlay Camera](camera-types-and-render-type-introduction.md#overlay-camera) that is part of the Base Camera’s [Camera Stack](camera-stacking.md), in the order defined in the Camera Stack:
