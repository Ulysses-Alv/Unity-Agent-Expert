...
A ColorField is an [Editor-only](UIB-interface-overview.md#enable-editor-extension-authoring-for-ui-documents-uxml) control that lets users select a color from a color picker. It supports alpha channel and [HDR colors](hdr-color-picker-reference.md). An alpha channel is an extra channel to represent the transparency level of a color. **HDR** colors are high-dynamic range colors that can represent a wider range of colors than standard colors.

![A ColorField control example](../uploads/Main/uxml/color-picker.png)

A ColorField control example
...
**C# class**: [`ColorField`](../ScriptReference/UIElements.ColorField.md)  
...
* [HDR Color Picker reference](hdr-color-picker-reference.md)
* [High dynamic range](hdr-landing.md)