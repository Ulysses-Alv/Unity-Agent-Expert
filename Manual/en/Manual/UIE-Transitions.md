...
You can use the controls in the [UI Builder](UIBuilder.md), a [USS file](UIE-USS.md), or a C# script to set the transition properties for a visual element.

The following table lists the transition properties and their corresponding C# methods:

| **Property** | **C# method** | **Description** |
| --- | --- | --- |
| `transition-property` | [`IStyle.transitionProperty`](../ScriptReference/UIElements.IStyle-transitionProperty.md) | Which USS properties the transition applies to. |
| `transition-duration` | [`IStyle.transitionDuration`](../ScriptReference/UIElements.IStyle-transitionDuration.md) | How long the transition takes to complete. |
| `transition-timing-function` | [`IStyle.transitionTimingFunction`](../ScriptReference/UIElements.IStyle-transitionTimingFunction.md) | How the property moves between values over time. |
| `transition-delay` | [`IStyle.transitionDelay`](../ScriptReference/UIElements.IStyle-transitionDelay.md) | When the transition starts. |
...
To learn how to trigger a transition with C# events, refer to [Create a simple transition with UI Builder and C# scripts](UIE-transition-example.md).
...
For visual elements in a hierarchy, USS transitions behave the same as [CSS transitions](https://drafts.csswg.org/css-transitions/#starting). If you set transitions for an inherited property, such as `color`, transitions applied to the parent elements cascade to the child elements. To find out which property is inherited, refer to [USS property reference](UIE-USS-Properties-Reference.md).
...
[Transition events](UIE-Transition-Events.md) are generated by transitions. You can use them to detect when a transition starts and ends. For an example, refer to [Create a transition event](UIE-transition-event-example.md).

### Usage hints

The **Usage Hints** offers a set of [properties](../ScriptReference/UIElements.UsageHints.md) for optimizations. You can use it to reduce draw calls and geometry regeneration.
...
To find out the animatability of each property, see [USS property reference](UIE-USS-Properties-Reference.md).

**Note**: It’s recommended that you use transitions with the [USS transform properties](UIE-Transform.md). Although you can use transitions on other USS properties, it might result in animations with low frame rates because value changes on these properties might cause layout recalculations. Layout recalculations in each frame can slow down the frame rate of your transition animation. All color properties, such as `color`, `background-color`, tint, and `opacity`, also have a fast pass that prevents the regeneration of the geometry.
...
The [`IStyle.transitionProperty`](../ScriptReference/UIElements.IStyle-transitionProperty.md) property is of type `StyleList<StylePropertyName>`. [`StylePropertyName`](../ScriptReference/UIElements.StylePropertyName.md) is a struct that you can [construct](../ScriptReference/UIElements.StylePropertyName-ctor.md) from a string. [`StyleList`](../ScriptReference/UIElements.StyleList_1.md) is a struct you can [construct](../ScriptReference/UIElements.StyleList_1-ctor.md) from a list of `StylePropertyName`.
...
The [`IStyle.transitionDuration`](../ScriptReference/UIElements.IStyle-transitionDuration.md) property is of type `StyleList<TimeValue>`. [`TimeValue`](../ScriptReference/UIElements.TimeValue.md) is a struct that you can [construct](../ScriptReference/UIElements.TimeValue-ctor.md) from a number and a [`TimeUnit`](../ScriptReference/UIElements.TimeUnit.md) enum. [`StyleList`](../ScriptReference/UIElements.StyleList_1.md) is a struct you can [construct](../ScriptReference/UIElements.StyleList_1-ctor.md) from a list of `TimeValue`.
...
The [`IStyle.transitionTimingFunction`](../ScriptReference/UIElements.IStyle-transitionTimingFunction.md) property is of type `StyleList<EasingFunction>`. [`EasingFunction`](../ScriptReference/UIElements.EasingFunction.md) is a struct that you can [construct](../ScriptReference/UIElements.EasingFunction-ctor.md) from an [`EasingMode`](../ScriptReference/UIElements.EasingMode.md) enum.
...
The [`IStyle.transitionDelay`](../ScriptReference/UIElements.IStyle-transitionDelay.md) property is of type `StyleList<TimeValue>`. [`TimeValue`](../ScriptReference/UIElements.TimeValue.md) is a struct that you can [construct](../ScriptReference/UIElements.TimeValue-ctor.md) from a number and a [`TimeUnit`](../ScriptReference/UIElements.TimeUnit.md) enum. [`StyleList`](../ScriptReference/UIElements.StyleList_1.md) is a struct you can [construct](../ScriptReference/UIElements.StyleList_1-ctor.md) from a list of `TimeValue`.
...
* [Use usage hints to reduce draw calls and geometry regeneration](UIE-performance-consideration-runtime.md)
