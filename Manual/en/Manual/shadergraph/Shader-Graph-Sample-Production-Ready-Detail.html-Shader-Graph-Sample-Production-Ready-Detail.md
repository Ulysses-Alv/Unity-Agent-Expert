# Production Ready Shaders | Shader Graph | 17.6.0

Source: [Original](https://docs.unity3d.com/Packages/com.unity.shadergraph@17.6/manual/Shader-Graph-Sample-Production-Ready-Detail.html)
...
This feature references a texture - Rock\_Macro\_NOS - which you are welcome to use, or you can create your own. The texture format is as follows:
...
This feature references a texture - Rock\_Micro\_NOS - which you are welcome to use, or you can create your own. The texture format is as follows:
...
The Deposition Moss module applies moss to the tops of the rocks. To define the moss, it uses a Moss\_CO texture (color with occlusion in the alpha channel) and a Moss\_N texture (normal). The alpha channel of the Moss\_CO texture is also used to create smoothness.
...
This shader uses the puddle\_norm texture. Notice that we save a little bit of shader performance by NOT storing this texture as a normal map. After the two samples are combined, then we expand the data to the -1 to 1 range, so we don’t have to do it twice.
...
This subgraph is very similar to the Rain\_Drips subgraph, but it’s adapted to work correctly for the RainOnTheLens post-process shader.
