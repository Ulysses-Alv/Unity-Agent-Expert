# Components and Blends | Shader Graph | 17.6.0

Source: [Original](https://docs.unity3d.com/Packages/com.unity.shadergraph@17.6/manual/Shader-Graph-Sample-Terrain-Components.html)
...
**Detail** - this node takes in material values from a regular terrain layer and combines them with a detail texture that uses the NOS [packing scheme](Shader-Graph-Sample-Terrain-Packing.html). It adds close-up, high frequency detail to a material and can be used in cases where large, individual pixels are visible in the base terrain because texture resolution isn’t high enough for the viewing distance.

**Detail CSNOH** - this subgraph node takes in material values from a CSNOH terrain layer and combines them with a detail texture that uses the NOS [packing scheme](Shader-Graph-Sample-Terrain-Packing.html). It adds close-up, high frequency detail to a material and can be used in cases where large, individual pixels are visible in the base terrain because texture resolution isn’t high enough for the viewing distance.
