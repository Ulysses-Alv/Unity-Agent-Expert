...
A ToolbarSpacer is an [Editor-only](UIB-interface-overview.md#enable-editor-extension-authoring-for-ui-documents-uxml) control that creates empty spaces within a [Toolbar](UIE-uxml-element-Toolbar.md) for layout control and visual separation of elements. It’s a [VisualElement](UIE-uxml-element-VisualElement.md) with a predefined style that matches the Toolbar style.
...
**C# class**: [`ToolbarSpacer`](../ScriptReference/UIElements.ToolbarSpacer.md)  
