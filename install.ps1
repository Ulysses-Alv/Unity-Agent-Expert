param([string]$ToolPath = "$env:LOCALAPPDATA\unity-expert\bin", [string]$Version = "latest")
$BINARY = "unity-expert.exe"
$REPO = "Ulysses-Alv/Unity-Agent-Expert"

if (Test-Path "$ToolPath\$BINARY") {
    & "$ToolPath\$BINARY" sync
    exit $LASTEXITCODE
}
...
$asset = (Invoke-RestMethod $apiUrl -Headers @{Accept = "application/vnd.github.v3+json"}).assets | Where-Object { $_.name -eq "unity-expert-windows-amd64.exe" }
