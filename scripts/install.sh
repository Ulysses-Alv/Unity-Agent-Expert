...
# Or with flags: curl -fsSL https://raw.githubusercontent.com/Ulysses-Alv/Unity-Agent-Expert/main/scripts/install.sh | bash -s -- --dry-run
#
# Flags:
...
show_help() {
    ...
    echo ""
    echo "One-liner: curl -fsSL https://raw.githubusercontent.com/Ulysses-Alv/Unity-Agent-Expert/main/scripts/install.sh | bash"
    echo ""
    echo "Options:"
...
get_repo_dir() {
    ...
    # Check if we're in /tmp or similar (ran via pipe)
    if [[ "$script_dir" == "/tmp"* ]]; then
        local tmp_dir="/tmp/unity-agent-expert-install-$$"
        log_info "Downloading installer to $tmp_dir..."
        if command -v git &> /dev/null; then
...
check_source_files() {
    ...
    if [ ! -d "${REPO_DIR}/${SKILLS_SRC}" ]; then
        log_warn "Skills folder not found (optional)"
    fi
