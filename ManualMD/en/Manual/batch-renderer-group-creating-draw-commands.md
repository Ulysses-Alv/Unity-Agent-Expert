...
To create draw commands, use the [BatchRendererGroup.OnPerformCulling](../ScriptReference/Rendering.BatchRendererGroup.OnPerformCulling.md) callback. Specifically, you use the callback’s [BatchCullingOutput](../ScriptReference/Rendering.BatchCullingOutput.md) parameter. This parameter contains a [NativeArray](../ScriptReference/Unity.Collections.NativeArray_1.md) with a single element. This layout means you can directly modify the contents of the array element without Unity copying data unnecessarily. The element in the `NativeArray` is a [BatchCullingOutputDrawCommands](../ScriptReference/Rendering.BatchCullingOutputDrawCommands.md) struct that contains the [actual draw commands](../ScriptReference/Rendering.BatchCullingOutputDrawCommands-drawCommands.md).
...
**Note**: To provide maximum flexibility, Unity doesn’t preallocate the arrays in the `BatchCullingOutputDrawCommands` output struct and stores them as raw pointers so you can easily allocate them and use them from [Burst](https://docs.unity3d.com/Packages/com.unity.burst@latest) jobs. You must allocate the arrays using [UnsafeUtility.Malloc](../ScriptReference/Unity.Collections.LowLevel.Unsafe.UnsafeUtility.Malloc.md) with the the [Allocator.TempJob](../ScriptReference/Unity.Collections.Allocator.TempJob.md) allocator. The callback shouldn’t release the memory. Instead, Unity releases the memory after it finishes rendering using the draw commands.
...
public class SimpleBRGExample : MonoBehaviour
{
    ...
    { 
        // Dispose of the buffer to avoid memory leaks.
