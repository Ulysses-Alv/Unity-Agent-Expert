...
When a [script compilation](script-compilation.md) is triggered, the API updater runs automatically. For example, this occurs when you:
...
When running Unity in batch mode from the command line, use the `-accept-apiupdate` option to run the API updater. For more information, see [Command Line Arguments](CommandLineArguments.md).

## Logging

The API updater logs changes it makes to any assemblies to the [Editor log](log-files.md). To control how much information is logged, set the UNITY\_APIUPDATER\_LOG\_THRESHOLD environment variable to the desired log threshold and start Unity. For example:
