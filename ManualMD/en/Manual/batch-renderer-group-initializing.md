...
The first step to render using BRG is to create an instance of [BatchRendererGroup](../ScriptReference/Rendering.BatchRendererGroup.md) and initialize it with an implementation of [OnPerformCulling](../ScriptReference/Rendering.BatchRendererGroup.OnPerformCulling.md).

The OnPerformCulling callback is the main entry point of BRG and Unity calls it whenever it culls visible objects. For information on the parameters it receives, see [OnPerformCulling](../ScriptReference/Rendering.BatchRendererGroup.OnPerformCulling.md). Typically, there are two tasks that the OnPerformCulling callback needs to perform:

* Visibility culling to determine which of its instances are visible based on the [BatchCullingContext](../ScriptReference/Rendering.BatchCullingContext.md) parameter.
* Output the actual draw commands to render those instances. To do this you write to the [BatchCullingOutput](../ScriptReference/Rendering.BatchCullingOutput.md) parameter.

In simple implementations, you can do these tasks directly in the OnPerformCulling callback, but for high-performance implementations, it’s best practice to do most of this work in [Burst](https://docs.unity3d.com/Packages/com.unity.burst@latest) jobs. The OnPerformCulling callback should return a [JobHandle](../ScriptReference/Unity.Jobs.JobHandle.md) that completes after the jobs write the output into the [BatchCullingOutput](../ScriptReference/Rendering.BatchCullingOutput.md) parameter. If your implementation doesn’t use jobs, you can return an empty JobHandle.
