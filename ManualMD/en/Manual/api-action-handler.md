...
To register an action, you create a function tagged with the [`SearchActionsProvider`](../ScriptReference/Search.SearchActionsProviderAttribute.md) attribute. This function must return an `IEnumerable<SearchAction>`.
...
The [`SearchAction`](../ScriptReference/SearchAction.md) class describes an action and provides a handler to execute the action on a specific `SearchItem`.
