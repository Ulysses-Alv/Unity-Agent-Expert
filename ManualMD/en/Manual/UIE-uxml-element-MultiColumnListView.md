...
![MultiColumnListView example](../uploads/Main/planets-multicolumnlistview.png)
...
The following example demonstrates how to bind a MultiColumnListView to a SerializedObject. For an example of runtime binding, refer to [Bind ListView to a list with runtime binding](UIE-runtime-binding-list-view.md).

If you have a ScriptableObject with the following structure:
...
![The MultiColumnListView with the Name and Moves columns](../uploads/Main/uxml/MultiColumnListView-binding.png)

The MultiColumnListView with the Name and Moves columns
...
![The MultiColumnListView with the updated Moves Column](../uploads/Main/uxml/MultiColumnListView-binding1.png)

The MultiColumnListView with the updated Moves Column
...
To enable sorting using the default algorithm, set the [`sortingMode`](../ScriptReference/UIElements.MultiColumnListView-sortingMode.md) attribute to [`ColumnSortingMode.Default`](../ScriptReference/UIElements.ColumnSortingMode.Default.md), and implement the [`Column.comparison`](../ScriptReference/UIElements.Column-comparison.md) to compare two items by their index in the source.

For an example, refer to [a MultiColumnListView default sorting example](../ScriptReference/UIElements.Column-comparison.md).

## Custom sorting

To implement custom sorting, set the [`sortingMode`](../ScriptReference/UIElements.MultiColumnListView-sortingMode.md) attribute to [`ColumnSortingMode.Custom`](../ScriptReference/UIElements.ColumnSortingMode.Custom.md), and implement the [`columnSortingChanged`](../ScriptReference/UIElements.MultiColumnListView-columnSortingChanged.md) callback. In the callback, use [`sortColumns`](../ScriptReference/UIElements.MultiColumnListView-sortedColumns.md) to reorder your list accordingly and trigger a [`RefreshItems()`](../ScriptReference/UIElements.BaseVerticalCollectionView.RefreshItems.md) once the items’ source has been updated.
...
**C# class**: [`MultiColumnListView`](../ScriptReference/UIElements.MultiColumnListView.md)  
**Namespace**: `UnityEngine.UIElements`  
**Base class**: [`BaseListView`](../ScriptReference/UIElements.BaseListView.md)
...
| `binding-source-selection-mode` | [`UIElements.BindingSourceSelectionMode`](../ScriptReference/UIElements.BindingSourceSelectionMode.md) | This property controls whether every element in the list will get its data source setup automatically to the correct item in the collection’s source.  When set to `AutoAssign`, the bind callbacks don’t need to be specified, since bindings can be used to fill the elements. |
...
| `reorder-mode` | [`UIElements.ListViewReorderMode`](../ScriptReference/UIElements.ListViewReorderMode.md) | This property controls the drag and drop mode for the list view.  The default value is `Simple`. When this property is set to `Animated`, Unity adds drag handles in front of every item and the drag and drop manipulation pushes items with an animation when the reordering happens. Multiple item reordering is only supported with the `Simple` drag mode. |
...
* [Bind ListView to a list with runtime binding](UIE-runtime-binding-list-view.md)