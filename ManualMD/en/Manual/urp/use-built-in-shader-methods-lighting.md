...
| `GetAdditionalLightsCount` | `int GetAdditionalLightsCount()` | Returns the number of additional lights. **Note**: this function always returns 0 in the Forward+ **rendering path**. For more information, refer to [Iterate over additional lights in the Forward+ rendering path](use-built-in-shader-methods-additional-lights-fplus.md). |
...
* [Use additional lights in the Forward+ rendering path](use-built-in-shader-methods-additional-lights-fplus.md)