...
GameObjects can be [static](StaticObjects.md), or dynamic (not static). Static **GameObjects** and dynamic GameObjects behave differently in Unity’s **occlusion culling** system:
...
With the Umbra occlusion system, the [GPU Resident Drawer](urp/gpu-resident-drawer.md) treats all objects as if dynamic occlusion were enabled while also providing its own GPU-based occlusion culling mechanism.