...
The density slider in the [Properties](SL-Properties.md) block controls how dense the checkerboard is. In the **vertex shader**, the mesh UVs are multiplied by the density value to take them from a range of 0 to 1 to a range of 0 to density. Let’s say the density was set to 30 - this will make **i.uv** input into the fragment shader contain floating point values from zero to 30 for various places of the mesh being rendered.
